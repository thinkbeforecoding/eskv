...
let parcelUrl =
    cmd.TryGetResult Parcel |> Option.defaultValue "http://localhost:1234"

let endpoint =
    cmd.TryGetResult EndPoint |> Option.defaultValue "http://localhost:5000"
...
type Entry =
    ...
    interface IEquatable<Entry> with
        member this.Equals(other) = this.Etag = other.Etag
...
let writeEvent (ws: WebSocket) (e: Event) =
    task {
        use s = new IO.MemoryStream()
        use w = new IO.StreamWriter(s)

        match e with
        | Event.Record e ->
            w.WriteLine($"ESKV-Event-Type: %s{e.Event.Type}")
            w.WriteLine($"ESKV-Event-Number: %d{e.EventNumber}")
            w.WriteLine()
            w.Flush()
            s.Write(e.Event.Data)
        | Event.Link l ->
            w.WriteLine($"ESKV-Event-Type: %s{l.OriginEvent.Event.Type}")
            w.WriteLine($"ESKV-Event-Number: %d{l.EventNumber}")
            w.WriteLine($"ESKV-Origin-Event-Number: %d{l.OriginEvent.EventNumber}")
            w.WriteLine($"ESKV-Origin-Stream: %s{l.OriginEvent.StreamId}")
            w.WriteLine()
            w.Flush()
            s.Write(l.OriginEvent.Event.Data)

        do! ws.SendAsync(s.ToArray().AsMemory(), WebSocketMessageType.Binary, true, cts.Token)
    }
...
if not isDevMode then
    builder.Environment.WebRootFileProvider <-
        new EmbeddedFileProvider(Reflection.Assembly.GetExecutingAssembly(), "eskv.wwwroot")
...
app.MapPut(
    ...
    fun ctx ->
        task {
            ...
            if
                ctx.Request.Headers.IfNoneMatch.Count = 1
                && ctx.Request.Headers.IfNoneMatch.[0] = "*"
            then
                ...
                if etags.Count = 1 then
                    ...
                    then
                        ...
                        do!
                            publish (
                                Shared.KeyChanged(containerKey, key, (Text.Encoding.UTF8.GetString(bytes), newEtag))
                            )
                    else
                        ctx.Response.StatusCode <- 409 // conflict
        ...
        task {
            ...
            | true, container ->
                if etags.Count = 1 then
                    ...
                    ctx.Response.StatusCode <- 204
                    do! publish (Shared.KeyDeleted(containerKey, key))
            | false, _ -> ctx.Response.StatusCode <- 204
...
let decode (bytes: byte[]) (contentType: string) =
    ...
    | true, ct ->
        if ct.MediaType.StartsWith("text/") then
            ...
            | "application/json"
            | "application/xml" -> Text.Encoding.UTF8.GetString(bytes)
            | _ -> null
    ...
    fun ctx ->
        task {
            if ctx.WebSockets.IsWebSocketRequest then
                ...
                use buffer = MemoryPool<byte>.Shared.Rent (4096)
        ...
        task {
            ...
            let expectedVersion =
                ...
                if h.Count = 0 then
                    ...
                    let v = int h.[0]
                    if v < -2 then ValueNone else ValueSome v
            ...
            | ValueSome expectedVersion ->
                let boundary =
                    HeaderUtilities
                        .RemoveQuotes(
                            MediaTypeHeaderValue.Parse(ctx.Request.ContentType).Boundary
                        )
                        .Value
                            ...
                            StreamUpdated(
                                ...
                                |> Array.map (fun e ->
                                    { StreamId = e.StreamId
                                      ...
                                      EventData = decode e.Event.Data e.Event.ContentType })
                            )
                        )
