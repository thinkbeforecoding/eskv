...
type Hasher(h: IncrementalHash, options: HasherOptions) =
    ...
    member _.Add (x: ReadOnlySpan<byte>) =
          h.AppendData(x)
    ...
    member this.ComputeHash(m: Reflection.MethodInfo) =
        ...
        let rec addExpr =
            ...
            | Patterns.WithValue(v,t,e) ->
                ...
                addExpr e
            | x ->
                failwithf "Unsupported expression: %A" x
        and addMethodOrDef(m: Reflection.MethodInfo) =
            match Expr.TryGetReflectedDefinition(m) with
            | Some e -> 
                if visitedMethods.Add m then
                    this.AddMethod m
                else
                    addExpr e
            | None ->this.AddMethod m
        and addTarget(t: Expr option) = 
            Option.iter addExpr t
    ...
    static member private ComputeInternal (key: string, x: Expr<_ -> _>, options) =
        ...
        let rec findCall =
            ...
            | Lambda(_, body) ->
                findCall body
            | expr -> 
                failwithf "Unsupported expression %A. Pass a function" expr
