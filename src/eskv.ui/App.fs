...
let update (command: Command) (model: Model) =
    match command with
    | ChangeNewKey txt -> { model with NewKey = txt }, Cmd.none
    | ChangeNewValue txt -> { model with NewValue = txt }, Cmd.none
    | CreateNew ->
    ...
    | OpenNew -> { model with ShowCreateNew = true }, Cmd.none
    | CloseNew -> { model with ShowCreateNew = false }, Cmd.none
    | Saved ->
        { model with
            NewKey = ""
            NewValue = ""
        ...
        Cmd.OfAsync.result (
            async {
                let! response =
                    fetch
                        $"/kv/{container}/{key}"
                        [ requestHeaders [ IfMatch etag ]; RequestProperties.Method HttpMethod.DELETE ]
                    |> Async.AwaitPromise
    ...
    | DeleteContainer(container) ->
        ...
        Cmd.OfAsync.result (
            async {
                let! response =
                    fetch $"/kv/{container}" [ RequestProperties.Method HttpMethod.DELETE ]
                    |> Async.AwaitPromise

                return Saved
            }
        )
    | Deleted -> { model with Saving = false }, Cmd.none
    | Edit edit -> { model with Editing = Editing edit }, Cmd.none
    | EditChanged text ->
        match model.Editing with
        | Editing edit -> { model with Editing = Editing { edit with Value = text } }, Cmd.none
        | _ -> model, Cmd.none

    | CancelEdit -> { model with Editing = NoEdit }, Cmd.none
    | SaveEdit ->
        match model.Editing with
        | Editing edit ->
            { model with Editing = Editing { edit with Editing.Saving = true } },
            Cmd.OfAsync.result (
                async {
                    let! response =
                        fetch
                            $"/kv/{edit.Container}/{edit.Key}"
                                ...
                                [ prop.style
                                  ...
                                  prop.children
                                      [ Bulma.navbarItem.div
                                            [ prop.style [ style.alignItems.stretch ]
                                              prop.children
                                                  [ Html.div
                                                        [ prop.text "eskv"
                    ...
                    [ Bulma.panelHeading
                          [ Html.div
                                [ prop.style
                                  ...
                                  prop.children
                                      [ Html.div
                                            [ prop.style [ style.flexGrow 1; style.textAlign.center ]
                                              prop.text "key / value" ]
                                        Html.a
                      ...
                      table.fixed'
                          [ if model.ShowCreateNew || not (Map.isEmpty model.Keys) then
                                Html.tr
                                    [ Html.th
                                          [ prop.text "Key"; prop.style [ style.width (length.calc ("30% - 5em")) ] ]
                                      Html.th
                                          [ prop.text "Value"; prop.style [ style.width (length.calc ("70% - 5em")) ] ]
                                      Html.th [ prop.style [ style.width (length.em 5) ] ] ]
                                          ...
                                          [ Bulma.input.text
                                                [ prop.disabled model.Saving
                                                  ...
                                                  prop.onKeyDown (fun e ->
                                                      match e.key with
                                                      | "Enter" -> dispatch CreateNew
                                                      | "Escape" -> dispatch CloseNew
                                                      | _ -> ())
                                                ...
                                                [ if model.NewKey = "" then
                                                      ...
                                                      Html.a
                                                          [
                                                            ...
                                                            prop.children
                                                                [ Bulma.icon [ Html.i [ prop.className "fas fa-plus" ] ] ]
                            ...
                            if not model.ShowCreateNew && Map.isEmpty model.Keys then
                                ...
                                for container, keys in Map.toSeq model.Keys do
                                    ...
                                    for key, (v, etag) in Map.toSeq keys do
                                        Html.tr
                                            [ table.tdEllipsis [ prop.className "key"; prop.text key ]
                                                              ...
                                                              Html.a
                                                                  [ prop.children
                                                                        [ Bulma.icon
                                                                              [ Html.i [ prop.className "fas fa-trash" ] ] ]
                                                                    prop.onClick (fun _ ->
                                                                        dispatch (DeleteKey(container, key, etag))) ]
                                          ...
                                          Html.tr
                                              [ Html.th
                                                    [ prop.style [ style.width (length.calc "30% - 5em") ]
                                                      prop.text "Stream" ]
                                                Html.th [ prop.style [ style.width (length.em 5) ]; prop.text "Event#" ]
                                                Html.th
                                              ...
                                              Html.tr
                                                  [ table.tdEllipsis.a
                                                        [ prop.className "is-cell"
                                                          prop.text streamid
                                                          prop.onClick (fun _ ->
                                                              dispatch (ChangeEventView(Stream streamid))) ]
                                                    table.tdEllipsis
                                                        [ prop.text eventNumber
                                                          prop.style
                                                              [ style.textAlign.right
                                                                style.paddingRight (length.em 1) ] ]
                                                    table.tdEllipsis [ prop.className "is-cell"; prop.text eventType ]
                                                    table.tdEllipsis [ prop.className "is-cell"; prop.text eventData ] ]
                                      | Stream stream ->
                                          Html.tr
                                              [ Html.th [ prop.style [ style.width (length.em 5) ]; prop.text "Event#" ]
                                                Html.th
                                          ...
                                          for _, eventNumber, eventType, eventData in
                                              model.Events |> List.filter (fun (id, _, _, _) -> id = stream) |> List.rev do
                                              Html.tr
                                                  [ Html.td
                                                        [ prop.text eventNumber
                                                          prop.style
                          ...
                          [ Bulma.text.hasTextCentered
                            prop.children
                                [ Html.a
                                  ...
                                  Html.text " "
                                  Html.a
                                      [ prop.children [ Bulma.icon [ Html.i [ prop.className "fab fa-twitter" ] ] ]
